import { nominatimSearchUrl, storedLabelFromNominatimPlace } from './nominatim.js';
import {
    TAGGED_FOLDER_KEY,
    MONTHS,
    daysInMonth,
...
class ProfileViewModel {
    ...
    initLocationMap = () => {
        ...
        addOpenStreetMapTiles(this._locationMap);
    ...
    runLocationSearch = async () => {
        ...
        try {
            const res = await fetch(nominatimSearchUrl(q), { headers: { Accept: 'application/json' } });
            const data = await res.json();
    ...
    pickLocationSearchResult = (place) => {
        ...
        this.locationSearchQuery(storedLabelFromNominatimPlace(place));
    ...
    pickNewEventLocationResult = (place) => {
        ...
        if (Number.isNaN(lat) || Number.isNaN(lng)) return;
        this.newEventLocationSearchResults([]);
        const name = storedLabelFromNominatimPlace(place);
    ...
    pickEditPhotoLocationSearchResult = (place) => {
        ...
        this.editPhotoLocationSearchResults([]);
        this.editPhotoLocationSearchQuery(storedLabelFromNominatimPlace(place));
    ...
    initEditPhotoLocationMap = () => {
        ...
        addOpenStreetMapTiles(this._editPhotoLocationMap);
